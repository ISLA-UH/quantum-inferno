...
window_types: list = ['boxcar', 'hann', 'hamming', 'tukey', 'blackman', 'flattop']
...
if __name__ == "__main__":
    ...
    """
    Apply a taper window to the input signal to reduce spectral leakage.
    ...
    for c_, (w_name_, ax_) in enumerate(zip(window_types, axx)):
        ...
        win_taper = get_window(w_name_, time_dft_nd, fftbins=False)
        ...
        win_mean = abs(sum(win_taper))/time_dft_nd
        win_var = sum(win_taper**2)/time_dft_nd
        win_rms = np.sqrt(win_var)


        print('\n *** Taper window: ', w_name_)
        print(' Input signal variance: ', sig_variance)
        print(' Tapered signal variance: ', sig_tapered_var)
        print(' Taper window mean amp: ', win_mean)
        print(' Taper window rms: ', win_rms)
        print(' Taper amplitude conversion factor (ACF=1/mean): ', 1/win_mean)
        print(' Taper energy conversion factor (ECF=1/rms): ', 1/win_rms)
    ...
    dft_sig_tapered_pos = np.fft.rfft(sig_cosine_tapered, n=time_dft_nd)
    # Compute the Real FFT of the zero-padded or truncated record
    fft_sig_pos = np.fft.rfft(sig_cosine, n=time_fft_nd)
    fft_sig_tapered_pos = np.fft.rfft(sig_cosine_tapered, n=time_fft_nd)
