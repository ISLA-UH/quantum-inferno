...
class TestPicker(unittest.TestCase):
    ...
    def test_scale_signal_by_extraction_type_max(self):
        scaled_signal = picker.scale_signal_by_extraction_type(self.signal, "sigmax")
        self.assertEqual(len(scaled_signal), 8192)
        self.assertEqual(scaled_signal[0], 1.0)
        self.assertAlmostEqual(scaled_signal[-1], 0.89, 2)

    def test_scale_signal_by_extraction_type_min(self):
        scaled_signal = picker.scale_signal_by_extraction_type(self.signal, "sigmin")
        self.assertEqual(len(scaled_signal), 8192)
        self.assertEqual(scaled_signal[0], 1.0)
        self.assertAlmostEqual(scaled_signal[-1], 0.89, 2)

    def test_scale_signal_by_extraction_type_abs(self):
        scaled_signal = picker.scale_signal_by_extraction_type(self.signal, "sigabs")
        self.assertEqual(len(scaled_signal), 8192)
        self.assertEqual(scaled_signal[0], 1.0)
        self.assertAlmostEqual(scaled_signal[-1], 0.89, 2)

    def test_scale_signal_by_extraction_type_log2(self):
        scaled_signal = picker.scale_signal_by_extraction_type(self.signal, "log2")
        self.assertEqual(len(scaled_signal), 8192)
    ...
    def test_scale_signal_by_extraction_type_log2max(self):
        scaled_signal = picker.scale_signal_by_extraction_type(self.signal, "log2max")
        self.assertEqual(len(scaled_signal), 8192)
        self.assertAlmostEqual(scaled_signal[0], 3.2e-16, 2)
        self.assertAlmostEqual(scaled_signal[-1], -0.16, 2)
    ...
    def test_find_peaks_by_extraction_type(self):
        result = picker.find_peaks_by_extraction_type(self.signal, "sigmax")
