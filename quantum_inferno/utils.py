...
def checkmake_dir(DIR_NAME):
    # Check export figure folder exist; if not, make
...
def resampled_mean_no_hop(sig_wf: np.array,
    ...
    new_sig_wf_mean = np.mean(new_wf_resampled, axis=1)

    # Resampled signal time
    new_sig_time_s = sig_time[0::resampling_factor]

    return [new_sig_time_s, new_sig_wf_mean]
...
def just_tile_d0(d0_array1d_in: Union[float, np.ndarray],
                 d0d1_shape: tuple) -> np.ndarray:
    """
    ...
    if len(d0d1_shape) == 1:
        ...
        if d0d1_shape[0] == len(d0_array1d_in):
            tiled_matrix = np.tile(d0_array1d_in, (d0d1_shape[1], 1)).T
    else:
        raise TypeError('Cannot handle an array of shape {}.'.format(str(d0_array1d_in.shape)))

    return tiled_matrix


def just_tile_d1(d1_array1d_in: Union[float, np.ndarray],
                 d0d1_shape: tuple) -> np.ndarray:
    """
    ...
    if len(d0d1_shape) == 1:
        ...
        raise TypeError('Cannot handle an array of shape {}.'.format(str(d1_array1d_in.shape)))
...
def d0tile_x_d0d1(d0: Union[float, np.ndarray],
    ...
    if len(shape_out) == 1:
        ...
        raise TypeError('Cannot handle an array of shape {}.'.format(str(d0.shape)))
