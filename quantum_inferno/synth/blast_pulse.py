...
def gt_blast_period_center(time_center_s: np.ndarray, pseudo_period_s: float) -> np.ndarray:
    ...
    sigint_g17 = np.where((1. < tau) & (tau <= 1 + np.sqrt(6.)))  # GT balanced pulse
    p_gt[sigint1] = (1. - tau[sigint1])
    p_gt[sigint_g17] = 1./6. * (1. - tau[sigint_g17]) * (1. + np.sqrt(6) - tau[sigint_g17]) ** 2.
...
def gt_blast_center_fast(frequency_peak_hz: float = 6.3,
    ...
    return time_center_s, antialias_halfNyquist(sig_gt + sig_noise)
...
def gt_blast_center_noise_uneven(sensor_epoch_s: np.array,
    ...
    pseudo_period_s = 1 / frequency_center_hz if frequency_center_hz else time_duration_s / 4.
    ...
    return antialias_halfNyquist(sig_gt + sig_noise)
...
def gt_blast_derivative_period_center(time_center_s: np.ndarray,
    ...
    sigint_g17 = np.where((1. < tau) & (tau <= 1 + np.sqrt(6.)))  # GT balanced pulse
    p_gtd[sigint1] = -1.
    p_gtd[sigint_g17] = -1./6. * (3. + np.sqrt(6) - 3*tau[sigint_g17]) * (1. + np.sqrt(6) - tau[sigint_g17])
...
def gt_blast_integral_period_center(time_center_s: np.ndarray,
    ...
    p_gti[sigint_g17] = -tau[sigint_g17]/72. * (
            3 * tau[sigint_g17]**3 - 4 * (3 + 2 * np.sqrt(6)) * tau[sigint_g17]**2 +
            6 * (9 + 4 * np.sqrt(6)) * tau[sigint_g17] - 12 * (7 + 2 * np.sqrt(6)))
...
def gt_blast_center_integral_and_derivative(frequency_peak_hz,
    ...
    time_center_s -= time_center_s[-1] / 2.
    tau_center = time_center_s / (pseudo_period_s / 4.)
...
def gt_blast_ft(frequency_peak_hz: float,
    ...
    ft_g17_negative = np.exp(-1j * w_scaled * (1 + np.sqrt(6.))) / (3. * w_scaled**4.) * \
        (1j * w_scaled * np.sqrt(6.) + 3. + np.exp(1j * w_scaled * np.sqrt(6.))
         * (3. * w_scaled**2. + 1j * w_scaled * 2. * np.sqrt(6.) - 3.))
