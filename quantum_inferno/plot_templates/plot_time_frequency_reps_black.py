...
Originally taken from libquantum, modified for use in quantum_inferno
"""
import enum
...
class FigureAttributesBackInBlack(FigureAttributes):
    ...
    def __init__(
        self, fig_size_ratio=np.array([640, 400]), fontsize1_scale=5, fontsize2_scale=4, line_color="w", line_style="-"
    ):
...
def origin_time_correction(time_input: np.ndarray, start_time_epoch: float, units_time: str) -> Tuple[str, np.ndarray]:
    """
    Sanitize time
...
def plot_wf_wf_wf_vert(
    ...
    attributes_tfr=FigureAttributes(),
) -> None:
    """
...
def plot_wf_mesh_mesh_vert(
    ...
    start_time_epoch: float = 0,
    frequency_hz_ymin: float = scales_dyadic.Slice.FU,
    frequency_hz_ymax: float = scales_dyadic.Slice.F0HZ,
    waveform_color: str = "midnightblue",
    ...
    if mesh_panel_b_colormap_scaling == "auto":
        mesh_panel_b_color_min, mesh_panel_b_color_max = mesh_colormap_limits(
            mesh_panel_b_trf, mesh_panel_b_colormap_scaling, mesh_panel_b_color_range
        )
